...
const Lesson = ({ lessonProps }: any) => {
  ...
  return (
    <>
      ...
      <Link href="/j6k/[id]" as={`/j6k/${pageNumber - 1}`}>
...
export const getStaticProps = async ({ params }: any) => {
  const { id } = params
  const lessonProps = await fetch(`http://127.0.0.1:8000/api/lesson/j6k/${id}?format=json`).then((res) => res.json())
