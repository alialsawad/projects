...
const slug = 'japanese-conversations'
const Lesson = ({ lessonProps }: any) => {
  ...
  return (
    <>
      ...
      <Link href="/japanese-conversations/[id]" as={`/japanese-conversations/${pageNumber - 1}`}>
...
export const getStaticProps = async ({ params }: any) => {
  const { id } = params
  const lessonProps = await fetch(`http://127.0.0.1:8000/api/lesson/japanese-conversations/${id}?format=json`).then((res) => res.json())
