...
function addPage(page) {
  const path = page.replace('src/pages', '').replace('.page.js', '').replace('.page.mdx', '').replace('/index', '/')
...
async function generateSitemap() {
  ...
  const pages = await globby(['src/pages/**/*{.page.tsx,.page.mdx}', '!src/pages/_*.tsx', '!src/pages/api'])
