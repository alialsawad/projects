...
export const Carousel = ({ width, height, images, ...rest }) => {
  ...
  useEffect(() => {
    ...
    renderer.current.domElement.style.width = '100vw'
    renderer.current.domElement.style.height = '100vh'
  ...
  useEffect(() => {
    document.getElementById('modal_drag').setAttribute('draggable', false)
  }, [])
  ...
  return (
    <div className={styles.carousel} onKeyDown={handleKeyDown} {...rest}>
      <div id="modal_drag" className={styles.modal} data-dragging={dragging} onPointerDown={handlePointerDown} />
      <div className={styles.content}>
        <div className={styles.imageWrapper} style={cssProps({ aspectRatio: `${width} / ${height}` })}>
        ...
        <button className={styles.button} data-next={true} aria-label="Next slide" onClick={() => navigate({ direction: 1 })}>
          Loop
        </button>
      </div>
      {images.map((image, index) => (
        <div key={uuid()} className={styles.content_container} aria-pressed={index === imageIndex}>
          {image.content}
        </div>
      ))}
    </div>
